...
def prune_releases(
    ...
    *,
    ...
    candidates: list[
        tuple[datetime, str, dict[str, Any], dict[str, Any], dict[str, Any]]
    ] = []
    for endpoint in inventory.endpoints:
        ...
        if (
            template is None
            or _template_release_sha(template) != release_sha
        ):
            continue
        ...
        if (
            ...
            continue
        candidates.append((created, release_sha, endpoint, template, auth))

    retired_releases: set[str] = set()
    for _created, release_sha, endpoint, template, auth in sorted(
        candidates, key=lambda item: item[0]
    ):
        endpoint_id = _resource_id(endpoint.get("id"), "endpoint ID")
        template_id = _resource_id(template.get("id"), "template ID")
        auth_id = _resource_id(auth.get("id"), "registry auth ID")
        # Never delete a shared dependency, even if its name resembles ours.
        fresh = client.inventory()
        fresh_endpoint = _by_id(fresh.endpoints, endpoint_id)
        fresh_template = _by_id(fresh.templates, template_id)
        fresh_auth = _by_id(fresh.auths, auth_id)
        if (
            fresh_endpoint is None
            or fresh_template is None
            or fresh_auth is None
            or fresh_endpoint.get("name") != endpoint.get("name")
            or fresh_endpoint.get("templateId") != template_id
            or fresh_template.get("name") != template.get("name")
            or fresh_template.get("containerRegistryAuthId") != auth_id
            or fresh_auth.get("name") != auth.get("name")
            or _endpoint_release_sha(fresh_endpoint) != release_sha
            or _template_release_sha(fresh_template) != release_sha
            or _auth_release_sha(fresh_auth) != release_sha
            or (_created_at(fresh_endpoint.get("createdAt")) or now) > cutoff
        ):
            continue
        if any(
            ...
            continue
        current_workers_max = fresh_endpoint.get("workersMax")
        if current_workers_max not in (0, WORKERS_MAX):
            continue
        try:
            verify_endpoint(
                fresh_endpoint,
        ...
        if current_workers_max != 0:
            verify_endpoint_rest(
                client.zero_endpoint(fresh_endpoint),
                endpoint_id=endpoint_id,
                expected_name=str(endpoint.get("name")),
                template_id=template_id,
                workers_max=0,
        ...
        after_template = client.delete_template(
            _resource_name(template.get("name"), TEMPLATE_NAME, "template name")
        )
        ...
        retired_releases.add(release_sha)
...
def read_previous_endpoint(path: Path) -> str | None:
    try:
        ...
        raise RunpodReleaseError("cannot read the previous endpoint file") from error
    if not value:
        return None
